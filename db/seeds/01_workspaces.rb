...
Rails.logger.debug '  Creating workspaces...'

def self.workspaces_data
  [
    {
    ...
    },
    {
      ...
      settings: {
        'timezone' => 'America/San_Francisco', 'language' => 'en',
...
WORKSPACES_DATA.each do |data|
  ...
  Rails.logger.debug { "  Created workspace: #{ws.name} [#{ws.plan}]" }
end

Rails.logger.debug { "  Workspaces total: #{Workspace.count}" }
