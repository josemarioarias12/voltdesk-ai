...
import { getStats, getFeatures, getTrustMetrics, getFooterSections, TECH_STACK } from "./components/constants";
...
export default function LandingPage() {
  ...
  const TRUST_METRICS = getTrustMetrics(t);
  const FOOTER_SECTIONS = getFooterSections(t);
  const TECH_DESCRIPTIONS = t("techStack.descriptions", { returnObjects: true }) as Record<string, string>;
  const EXPLAINABILITY_ITEMS = t("explainability.items", { returnObjects: true }) as { title: string; desc: string; tag: string }[];
  const EXPLAINABILITY_SVGS = [
    `<svg viewBox="0 0 24 24" fill="none" stroke="#02C39A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12"r="3"/></svg>`,
    `<svg viewBox="0 0 24 24" fill="none" stroke="#02C39A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>`,
    `<svg viewBox="0 0 24 24" fill="none" stroke="#02C39A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polylinepoints="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>`,
  ];
  const CTA_BADGES = t("cta.badges", { returnObjects: true }) as [string, string][];
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif", background: "#fff", color: "#1E293B", overflowX: "hidden" }}>
      ...
      <section style={{ background: "#0D1B2A", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", paddingTop: 80, paddingBottom: 80, position: "relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", textAlign: "center", position: "relative", zIndex: 1, width: "100%" }}>
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ display: "inline-block", marginBottom: 32 }}>
            <motion.div
              animate={{ boxShadow: ["0 0 0 0 rgba(2,195,154,0.4)", "0 0 0 8px rgba(2,195,154,0)", "0 0 0 0rgba(2,195,154,0)"] }}
            ...
            >
              ...
              <span style={{ color: "#02C39A", fontSize: 13, fontWeight: 500 }}>{t("hero.badge")}</span>
            </motion.div>
          </motion.div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible">
            <motion.h1 variants={fadeUp} style={{ fontSize: "clamp(48px,7vw,88px)", fontWeight: 800, lineHeight: 1.05, color: "#fff", margin: "0 0 24px", letterSpacing: "-0.04em" }}>
              {t("hero.titleLead")}{" "}
              <span style={{ background: "linear-gradient(135deg,#028090 0%,#02C39A 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{t("hero.titleHighlight")}</span>
            </motion.h1>
            ...
            <motion.div variants={fadeUp} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24, flexWrap: "wrap" }}>
              <span style={{ color: "#64748b", fontSize: 13 }}>{t("hero.badge1")}</span>
              <span style={{ color: "#1e3a5f" }}>·</span>
              <span style={{ color: "#64748b", fontSize: 13 }}>{t("hero.badge2")}</span>
              <span style={{ color: "#1e3a5f" }}>·</span>
              <span style={{ color: "#64748b", fontSize: 13 }}>{t("hero.badge3")}</span>
            </motion.div>
          </motion.div>
          <DashboardMockup />
        </div>
      ...
      <section id="features" style={{ background: "#f8fafc", padding: "96px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: "center", marginBottom: 64 }}>
            <h2 style={{ fontSize: "clamp(28px,4vw,44px)", fontWeight: 800, color: "#0f172a", marginBottom:12, letterSpacing: "-0.02em" }}>{t("features.sectionTitle")}</h2>
            <p style={{ color: "#64748b", fontSize: 18 }}>{t("features.sectionSubtitle")}</p>
          </motion.div>
      ...
      <section id="demo" style={{ background: "#0D1B2A", padding: "112px 24px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <span style={{ color: "#02C39A", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em" }}>{t("explainability.poweredBy")}</span>
          </motion.div>
          ...
          <div className="xai-grid" style={{ display: "grid", gap: 80, alignItems: "center" }}>
            ...
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <h2 style={{ fontSize: "clamp(36px,5vw,64px)", fontWeight: 800, color: "#fff", marginBottom: 20, lineHeight: 1.05, letterSpacing: "-0.03em" }}>
                {t("explainability.titleLead")}<br />
                <span style={{ background: "linear-gradient(135deg,#028090,#02C39A)", WebkitBackgroundClip:"text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{t("explainability.titleHighlight")}</span>
              </h2>
              ...
              <div style={{ position: "relative" }}>
                ...
                {EXPLAINABILITY_ITEMS.map((item, i) => (
                  <motion.div
                    key={item.title}
                  ...
                  >
                    ...
                    <div style={{ flexShrink: 0, width: 40, height: 40, borderRadius: "50%", background: "rgba(2,195,154,0.1)", border: "1px solid rgba(2,195,154,0.3)", display: "flex", alignItems: "center", justifyContent:"center", boxShadow: "0 0 16px rgba(2,195,154,0.15)", zIndex: 1 }}
                      dangerouslySetInnerHTML={{ __html: EXPLAINABILITY_SVGS[i].replace('<svg', '<svg width="18" height="18"') }}
                    />
      ...
      <section style={{ background: "#0D1B2A", padding: "96px 24px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: "center", marginBottom: 64 }}>
            <p style={{ color: "#02C39A", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 12 }}>{t("trust.eyebrow")}</p>
            <h2 style={{ fontSize: "clamp(28px,4vw,44px)", fontWeight: 800, color: "#fff", letterSpacing: "-0.03em", marginBottom: 12 }}>{t("trust.title")}</h2>
            <p style={{ color: "#475569", fontSize: 17 }}>{t("trust.subtitle")}</p>
          </motion.div>
      ...
      <section id="pricing" style={{ background: "#0a0f1a", padding: "96px 24px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <p style={{ color: "#02C39A", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 12 }}>{t("enterprise.eyebrow")}</p>
          <h2 style={{ fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, color: "#fff", marginBottom: 20, letterSpacing: "-0.02em" }}>{t("enterprise.title")}</h2>
      ...
      <section id="api" style={{ background: "#f8fafc", padding: "96px 0", borderTop: "1px solid #e2e8f0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 72, padding: "0 24px" }}>
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <p style={{ color: "#028090", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 12 }}>{t("techStack.eyebrow")}</p>
            <h2 style={{ fontSize: "clamp(28px,3vw,44px)", fontWeight: 800, color: "#0f172a", marginBottom:12, letterSpacing: "-0.02em" }}>{t("techStack.title")}</h2>
            <p style={{ color: "#64748b", fontSize: 17 }}>{t("techStack.subtitle")}</p>
          </motion.div>
        </div>
      ...
      <section style={{ background: "#0D1B2A", padding: "120px 24px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 700, margin: "0 auto" }}>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.05 }}
            ...
            {t("cta.titleLine1")}<br />
            <span style={{ color: "#fff" }}>{t("cta.titleLine2")}</span>
          </motion.h2>
      ...
      {/* Footer */}
      <footer style={{ background: "#0D1B2A", borderTop: "1px solid rgba(255,255,255,0.08)", padding: "72px24px 32px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div className="footer-grid" style={{ display: "grid", gap: 32, marginBottom: 64 }}>
            <div>
              ...
              <p style={{ fontSize: 14, lineHeight: 1.8, color: "#475569", marginBottom: 16, maxWidth: 260 }}>{t("footer.tagline")}</p>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(2,195,154,0.08)", border: "1px solid rgba(2,195,154,0.2)", borderRadius: 20, padding: "6px 12px", width: "fit-content", marginBottom: 20 }}>
                ...
                <span style={{ color: "#02C39A", fontSize: 12, fontWeight: 600 }}>{t("footer.systemsOperational")}</span>
              </div>
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <a href="https://linkedin.com" target="_blank" rel="noreferrer"
                  ...
                  onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.04)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; }}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="#64748b"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 12.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
              </div>
            </div>
