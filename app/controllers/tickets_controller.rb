...
class TicketsController < ApplicationController
  ...
  def create
    ...
    if result.success?
      redirect_to ticket_path(result.data), notice: "Ticket #{result.data.ticket_number} created. AI classification started."
    else
      redirect_back fallback_location: new_ticket_path, alert: result.error
    end
  end
  ...
  def update
    ...
    if result.success?
      ...
      redirect_back fallback_location: ticket_path(@ticket), alert: result.error
    end
  end
  ...
  def ticket_params
    params.require(:ticket).permit(:title, :description, :department_id, :priority, :category, :source)
  end

  def ticket_update_params
    params.require(:ticket).permit(:title, :description, :priority, :status, :department_id, :assigned_to_id)
  end
  ...
  def serialize_ticket_detail(t)
    serialize_ticket(t).merge(
      comments:   policy_scope(t.comments).chronological.map { |c|
        { id: c.id, body: c.body, internal: c.internal, created_at: c.created_at.iso8601, user: user_stub(c.user) }
      },
      activities: t.activities.chronological.map { |a|
        { id: a.id, action: a.action, metadata: a.metadata, created_at: a.created_at.iso8601, user: a.user ? user_stub(a.user) : nil }
      }
    )
  end
